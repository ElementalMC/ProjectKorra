		...
		switch (this.ability) {
			...
			case GRAB:
				if (this.grabbedEntity == null) {
					for (final Entity entity : GeneralMethods.getEntitiesAroundPoint(location, 2)) {
						if (entity instanceof LivingEntity && !entity.getUniqueId().equals(this.player.getUniqueId()) && !GRABBED_ENTITIES.containsKey(entity)) {
							if (GeneralMethods.isRegionProtectedFromBuild(this, entity.getLocation()) || ((entity instanceof Player) && Commands.invincible.contains(((Player) entity).getName()))) {
								continue;
							}
	...
	public void remove() {
		...
		if (hasAbility(this.player, WaterArms.class)) {
			...
			if (this.hasDamaged) {
				...
				ChatUtil.sendActionBar(Element.WATER.getSubColor() + "Punches Left: " + this.waterArms.getMaxPunches(), this.player);
			} else {
				ChatUtil.sendActionBar(Element.WATER.getSubColor() + "Uses Left: " + this.waterArms.getMaxUses(), this.player);
			}
